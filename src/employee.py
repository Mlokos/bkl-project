...
def get_job_title_generalized(job_title: float, generalization: int, specialization: Optional[int]) -> float:
    str_value = str(job_title)

    jobs_range = generalization + len(str(specialization)) if specialization else generalization
...
def get_job_title_to_competencies(df: DataFrame, df_meta: metadata_container, generalization: int, specialization: Optional[int]) -> DataFrame:
