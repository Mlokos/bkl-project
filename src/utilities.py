...
def load_sav_data() -> Tuple[DataFrame, DataFrame]:
    ...
    if all(
        ...
        employee_sav_df, employee_sav_meta = read_sav(
            "datasets/Baza_danych_z_badania_ludnoci_BKL_edycja_2019.sav"
        )
        boss_sav_df, boss_sav_meta = read_sav(
            "datasets/Baza_danych_z_badania_pracodawcw_BKL_edycja_2019.sav"
        )
...
def load_spss_data() -> Tuple[DataFrame, DataFrame]:
    ...
    if all([path_employee.is_file(), path_boss.is_file()]):
        ...
        employee_spss_df = read_spss(
            "datasets/Baza_danych_z_badania_ludnoci_BKL_edycja_2019.sav"
        )
        boss_spss_df = read_spss(
            "datasets/Baza_danych_z_badania_pracodawcw_BKL_edycja_2019.sav"
        )
...
class SavAdapter:
    ...
    def get_value_name(
        self, column_name: str, value: Union[str, float]
    ) -> Union[str, float]:
